...
exports.addMDI = async (req, res) => {
    const { mdi_id,mdi_family_tree_id, document_no, rev_no, quantity_per_assembly, material_condition_form, material_standard,finish_surface_treatment,
        finish_standard,manufacturing_process,material_source,life_of_item,address,date_of_issue,document_path,document_name } = req.body;
    ...
    try {
        if (await mdiIdExists(mdi_id)) {
            ...
            const creatingMDI = await creationOfMDI.create({ mdi_id,mdi_family_tree_id, document_no, rev_no, quantity_per_assembly, material_condition_form, material_standard,finish_surface_treatment,
                finish_standard,manufacturing_process,material_source,life_of_item,address,date_of_issue,document_path,document_name , created_by:createdBy });
...
exports.updateMDI = async (req, res) => {
    ...
    try {
        ...
        if (!updateMDI) {
            ...
            if (await mdiIdExists(mdi_id)) {
                response = { status: 0, message: "MDI with the same  id already exists" };
            } else {
                const updatedBy = req.userId;
                await updateMDI.update({ mdi_id,mdi_family_tree_id, document_no, rev_no, quantity_per_assembly, material_condition_form, material_standard,finish_surface_treatment,
                    finish_standard,manufacturing_process,material_source,life_of_item,address,date_of_issue,document_path,document_name  , updated_by:updatedBy });
